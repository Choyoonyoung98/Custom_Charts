...
class BarChartView: UIView {
    ...
    init(frame: CGRect, values: [CGFloat], titles: [String], colors: [UIColor], animated: Bool) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let sum = values.reduce(0, { (first: CGFloat, second: CGFloat) -> CGFloat in
            return first + second
        })
        ...
        for i in 0..<values.count {
            ...
            let newPosition = CGPoint(x: currentX, y: self.frame.height * 7 / 8 - (self.frame.height * self.values[i]/sum))
            ...
            if(values[i] != 0.0) {
                ...
                percentDesciptionLabel = UILabel(frame: CGRect(x: currentX, y: self.frame.height * 7 / 8 - self.frame.height * self.values[i]/sum - 10, width: 100, height: 100))
                percentDesciptionLabel.text = "\(self.values[i] / sum * 100)%"
                percentDesciptionLabel.center = CGPoint(x: currentX, y: self.frame.height * 7 / 8 - self.frame.height * self.values[i]/sum - 10)
                ...
                repoNameDesciptionLabel = UILabel(frame: CGRect(x: currentX, y: self.frame.height - 10, width: 100, height: 100))
                repoNameDesciptionLabel.text = "\(titles[i])"
                repoNameDesciptionLabel.center = CGPoint(x: currentX, y: self.frame.height - 10)
